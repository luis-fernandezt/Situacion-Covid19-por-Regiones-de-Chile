...
gg1 <- ggplot(sdt_comunas, aes(x=Fecha, y=Casos_activos, group=Comuna, color=Comuna)) +
  ...
  labs(x = "Fecha", 
       ...
       subtitle = "18 de septiembre de 2020", 
...
gg2 <- ggplot() +
  ...
  geom_text_repel(data=sft_casos, size= 3, color= 'black', fontface = 'bold',  segment.color = NA,
                  hjust = 0.5, vjust  = 0.51, aes(coords_x, coords_y, label= Comuna)) +
  
  geom_label_repel(data=sft_casos, size= 3, color= 'white', fontface = 'bold', fill = 'red', segment.color = NA,
